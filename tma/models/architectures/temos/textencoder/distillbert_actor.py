...
class DistilbertActorAgnosticEncoder(DistilbertEncoderBase):
    ...
    def forward(self, texts: List[str]):
        ...
        if self.hparams.vae:
            mu_token2 = torch.tile(self.mu_token, (bs,)).reshape(bs, -1)
            logvar_token2 = torch.tile(self.logvar_token, (bs,)).reshape(bs, -1)

            # adding the distribution tokens for all sequences
            xseq = torch.cat((mu_token2[None], logvar_token2[None], x), 0)

            # create a bigger mask, to allow attend to mu and logvar
            token_mask = torch.ones((bs, 2), dtype=bool, device=x.device)
            aug_mask = torch.cat((token_mask, mask), 1)
        else:
            emb_token2 = torch.tile(self.emb_token, (bs,)).reshape(bs, -1)

            # adding the embedding token for all sequences
            xseq = torch.cat((emb_token2[None], x), 0)

...
class TEMOS(BaseModel):
    ...
    def save_embeddings(self, batch):
        ...
        with torch.no_grad():
            ...
            #print(text_all.shape,motion_all.shape,retrieval_name)
    ...
    def allsplit_step(self, split: str, batch, batch_idx):
        if split in ["train"]:
            return self.allsplit_step_single(split,batch,batch_idx)
        else:
            with torch.no_grad():
                return self.allsplit_step_single(split,batch,batch_idx)

    def allsplit_step_single(self, split: str, batch, batch_idx):
        ...
        # if split in ["test"]:
        #     return feat_from_motion, batch["length"], batch["text"] ## feat_from_text
    ...
    def allsplit_epoch_end(self, split: str, outputs):
        ...
        if split in ["val", "test"]:
            if (self.trainer.current_epoch + 1) % 100 == 0:
                ...
                # print(self.retrieval_text_embedding.shape,'T1')
                # print(self.retrieval_motion_embedding.shape,'M1')

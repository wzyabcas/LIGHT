...
class ClassifierFreeSampleModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None,is_causal=None,x2=None,x3=None,modes=None):
        ...
        out = self.model(x, timesteps, y,is_causal=is_causal,x2=x2,x3=x3,modes=modes)
        if self.cfg:
            out_uncond = self.model(x, timesteps, y_uncond,is_causal=is_causal,x2=x2,x3=x3,modes=modes)
            delta = out - out_uncond
            d1 = delta[:,:,:,0::3]
            d2 = delta[:,:,:,1::3]
            d3 = delta[:,:,:,2::3]
            # print(torch.norm(d1, p=2, dim=(1,2,3)),'N1')
            # print(torch.norm(d2, p=2, dim=(1,2,3)),'N2')
            # print(torch.norm(d3, p=2, dim=(1,2,3)),'N3')
...
class AutoRegressiveSampler():
    ...
    def sample(self, model, shape, **kargs):
        ...
        # if self.args.predict_prefix:
        #     autoregressive_shape[-1] = self.args.pred_len+self.args.context_len
        # else:
        ...
        for i in range(n_iterations):
            ...
            if self.args.predict_prefix:
                sample = sample[..., -self.args.pred_len:]
